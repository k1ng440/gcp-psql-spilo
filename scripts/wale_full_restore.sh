...
while true; do
	...
	[[ ! -z "$CONNSTR" && $server_version == "-1" ]] && server_version=$(psql -d "$CONNSTR" -tAc 'show server_version_num' 2>/dev/null || log "-1")
...
[[ -z $wal_segment_backup_start ]] && log "Can not find any backups" && exit 1

[[ -z $NO_MASTER && $server_version == "-1" ]] && log "Failed to reach master" && exit 1
...
if [[ $server_version != "-1" ]]; then
	...
	[[ -z $diff_in_bytes || -z $cluster_size ]] && log "could not determine difference with the master location" && exit 1

	log "Current cluster size: $cluster_size"
	log "Wals generated since the last backup: $diff_in_bytes"

	[[ $diff_in_bytes -gt $((THRESHOLD_MEGABYTES * 1048576)) ]] && log "Generated wal size exceeds threshold: ${THRESHOLD_MEGABYTES}MB, but restoring anyway"

	# It says that if 30% (100G) of the cluster is smaller than the size delta (4G), bail out
	# 5G cluster dize, wals generated = 8G
	readonly threshold_bytes=$((cluster_size * THRESHOLD_PERCENTAGE / 100))
	[[ $threshold_bytes -lt $diff_in_bytes ]] && log "Generated wal size exceeds cluster size threshold: ${THRESHOLD_PERCENTAGE}%, but restoring anyway"
fi
